...
int main(int argc, char* argv[]) {
    ...
    //PipeSketch
    HeavyChanger<PipeSketch>* heavychanger = new HeavyChanger<PipeSketch>(lgwidth,lgdepth,rgwidth,rgdepth,hvwidth,hvdepth);//模板类的使用
    for (std::string file; getline(tracefiles, file);) {
        ...
        //Reset gounrdtmp; groundtmp->key[0]计数 groundtnp->second[0]=0
        ...
        if (numfile != 0) {
            ...
            for (auto it = groundtmp.begin(); it != groundtmp.end(); it++) {
                ...
                if (diff > thresh*(diffsum)){
                    // std::cout<<"heavy changer:"<<diff<<std::endl;
                    ground[it->first] = diff;
                }
            }
            std::cout<<"heavy changer thresh:"<<thresh*(diffsum)<<std::endl;
            std::cout<<"heavy changer numbers"<<ground.size()<<std::endl;
        }
        ...
        adaptor->Reset();
        heavychanger->Reset();
        PipeSketch* cursk = (PipeSketch*)heavychanger->GetCurSketch();
        t1 = now_us();
        while(adaptor->GetNext(&t) == 1) {
            cursk->insert((unsigned char*)&(t.key),t.size);
        }
        t2 = now_us();
        ...
        if (numfile != 0) {
            ...
            // t1 = now_us();
            heavychanger->query(threshold, results);
            // t2 = now_us();
                ...
                if (it->second > threshold) {
                    ...
                    for (auto res = results.begin(); res != results.end(); res++) {
                        if (memcmp(it->first.key, res->first.key, sizeof(res->first.key)) == 0) {
                            std::cout<<"diff:"<<int(res->second - it->second)<<std::endl;
                            error += std::abs(int(res->second - it->second))*1.0;///it->second;
            ...
            recall = tp*1.0/cnt;
            error = error/tp;
            std::cout<<"hc_precision:"<<precision<<std::endl;
            std::cout<<"hc_recall:"<<recall<<std::endl;
            std::cout<<"hc_error:"<<error<<std::endl;
            std::cout<<"est heavy changer numbers"<<results.size()<<std::endl;
        }
