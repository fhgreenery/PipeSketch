...
public:
    ...
    void insert(uint8_t *key, int32_t f) //int f = 1
    {
    ...
    void quick_insert(uint8_t *key, int f)//int f = 1
    {
    ...
    void get_heavy_hitters(int threshold, vector<pair<string, int>> & results)
    {
        for (int i = 0; i < bucket_num; ++i) 
            for (int j = 0; j < MAX_VALID_COUNTER; ++j) 
            {
                ...
                if (val >= threshold) {
                    results.push_back(make_pair(string((const char*)&key, 13), val));
                }
            }
    }
...
/* interface */
    ...
    {
        constexpr uint32_t alignment = 136;
