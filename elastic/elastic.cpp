...
#define BUCKET_NUM (HEAVY_MEM / 136) //5-tuple+pkt_size
#define TOT_MEM_IN_BYTES (600 * 1024)
	...
	for(int datafileCnt = START_FILE_NO; datafileCnt <= END_FILE_NO; ++datafileCnt)
	{
		...
		{ 	
			elastic->insert((uint8_t*)(traces[datafileCnt - 1][i].key), 1); //int32_t pkt_size)

			string str((const char*)(traces[datafileCnt - 1][i].key), 13);
			Real_Freq[str]++;
		}
