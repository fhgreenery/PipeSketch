...
MVSketch::MVSketch(int depth, int width, int lgn) {
    ...
    // char name[] = "MVSketch";
    //AwareHash中三个数字表示hash,scale,hardener的值
...
void MVSketch::Update(unsigned char* key, val_tp val) {
    ...
    int keylen = mv_.lgn/8; //转化成整型
    for (int i = 0; i < mv_.depth; i++) {
        bucket = MurmurHash64A(key, keylen, mv_.seed[i]) % mv_.width;//mv_.hardner[i]
        int index = i*mv_.width+bucket;
        MVSketch::SBucket *sbucket = mv_.counts[index];
        sbucket->sum += val;
        //如果sbucket的key为空
        if (sbucket->key[0] == '\0') {
            //给sbucket的key和count赋值，从key中拷贝keylen个字节到sbucket的key中
            memcpy(sbucket->key, key, keylen);
            sbucket->count = val;
        } else if(memcmp(key, sbucket->key, keylen) == 0) {
            sbucket->count += val; //k=x
        } else {
            sbucket->count -= val;
...
void MVSketch::Query(val_tp thresh, std::vector<std::pair<key_tp, val_tp> >&results) {
    //筛选1：把sketch中每个bucket sum超过阈值的key复制到res中
    myset res;
    for (int i = 0; i < mv_.width*mv_.depth; i++) {
        if (mv_.counts[i]->sum > thresh) {
            key_tp reskey;//创建一个临时变量
            memcpy(reskey.key, mv_.counts[i]->key, mv_.lgn/8);
            res.insert(reskey);
        }
    }
    //筛选2:估算value值，存储超过阈值的key，value.
    for (auto it = res.begin(); it != res.end(); it++) {
        val_tp resval = 0;
        for (int j = 0; j < mv_.depth; j++) {
            unsigned long bucket = MurmurHash64A((*it).key, mv_.lgn/8, mv_.seed[j]) % mv_.width;
            ...
            else resval = std::min(tempval, resval);//取多个hash值中的最小值
        }
        if (resval > thresh ) {
            key_tp key;
...
val_tp MVSketch::Low_estimate(unsigned char* key) {
    val_tp ret = 0;
    for (int i = 0; i < mv_.depth; i++) {
        unsigned long bucket = MurmurHash64A(key, mv_.lgn/8, mv_.seed[i]) % mv_.width;
